...
int main(int argc, char **argv) {
  ...
  n = find_length(head);
  //printf("user: %s\ntitle: %s\nviews: %d\ntime: %s\n\n\n", head->user, head->title, head->views, head->duration);
  printf("%d clips\n",n);
  return 0;
}
...
struct clip *append(struct clip **hp,char **four) {
  struct clip *cp,*tp;
   ...
   tp->user = malloc(sizeof(four[0]));
   tp->duration = malloc(sizeof(four[1]));
   tp->title = malloc(sizeof(four[3]));

   // strcpy three strings to tp
   strcpy(tp->user, four[0]);
   strcpy(tp->duration, four[1]);
